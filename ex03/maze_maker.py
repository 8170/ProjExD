...
def change_color(number):
    color_lst = ["red1","SkyBlue","violet","gold","yellow1","SpringGreen2"]
    if number == 1:
        return "black"
    else:
        return random.choice(color_lst)
...
def show_maze(canvas, maze_lst):
    for y in range(len(maze_lst)):
        for x in range(len(maze_lst[y])):
            canvas.create_rectangle(x*100, y*100, x*100+100, y*100+100, fill=change_color(maze_lst[y][x]))#fill=color[maze_lst[y][x]]
